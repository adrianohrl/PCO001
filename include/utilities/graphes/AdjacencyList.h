...
template<typename T>
std::list<T> AdjacencyList<T>::getVertices() const
{
	std::list<T> vertices;
...
template<typename T>
std::list<std::pair<T, double> > AdjacencyList<T>::getArcs(const T &content) const
{
	return getArcs(getVertex(content));
}

template<typename T>
std::list<std::pair<T, double> > AdjacencyList<T>::getArcs(Vertex<T> *vertex) const
{
	if (!vertex)
	{
...
template<typename T>
T *AdjacencyList<T>::getContentReference(const T &content)
{
	return getVertex(content)->getContentReference();
}

template<typename T>
std::size_t AdjacencyList<T>::getSize() const
{
	return size_;
}

template<typename T>
bool AdjacencyList<T>::isDirected() const
{
	return directed_;
}

template<typename T>
bool AdjacencyList<T>::isAcyclic()
{
	if (isEmpty())
	{
		return false;
...
template<typename T>
bool AdjacencyList<T>::isEmpty() const
{
	return size_ == 0;
}
...
template<typename T>
bool AdjacencyList<T>::isDisconnected(Vertex<T> *origin, Vertex<T> *destiny) const
{
	if (!origin || !destiny)
	{
		return false;
...
template<typename T>
bool AdjacencyList<T>::hasVertex(const T &content) const
{
...
template<typename T>
bool AdjacencyList<T>::connect(Vertex<T> *origin, Vertex<T> *destiny, double weight)
{
	if (!origin || !destiny)
	{
		return false;
...
template<typename T>
Vertex<T> *AdjacencyList<T>::get(std::size_t index)
{
	if (index >= size_)
	{
		return NULL;
...
template<typename T>
std::queue<T> AdjacencyList<T>::getTopologicalOrdering()
{
...
template<typename T>
std::list<std::pair<T, double> > AdjacencyList<T>::getPredecessors(const T &content)
{
	reset();
...
template<typename T>
std::list<std::pair<T, double> > AdjacencyList<T>::getPredecessors(Vertex<T> *vertex) const
{
	// nao esta implementado da forma mais eficiente. Seria melhor mudar a estrutura de Vertex.
	// ao inves dessa classe ter uma lista de vertices adjacentes, ela teria uma lista de
	// vertices sucessores e outra de vertices predecessores.

	// ou fazer controle do grau de entrada e saida de cada vertice (o q e mais facil de implementar);
	...
	for (int i(0); i < size_; i++)
	{
		...
		while (it != successors.end())
		{
			...
			{
				predecessors.push_back(std::pair<T, double>(vertices_[i]->getContent(), it->second));
			}
			it++;
		}
	}
...
template<typename T>
std::list<std::pair<T, double> > AdjacencyList<T>::getSuccessors(const T &content) const
{
	return getArcs(getVertex(content));
}

template<typename T>
std::list<std::pair<T, double> > AdjacencyList<T>::getSuccessors(Vertex<T> *vertex) const
{
