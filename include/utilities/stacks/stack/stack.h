...
Stack<Element>::Stack(const int max_size)
    : max_size_(max_size)
{
  size_ = 0;
  top_ = NULL;
...
template <typename Element>
Stack<Element>::Stack(const Stack& stack)
    : max_size_(stack.max_size_)
{
  ...
  if (top_)
  {
    top_ = new Node<Element>(*stack.top_);
  }
}
...
template <typename Element> bool Stack<Element>::isEmpty() { return !top_; }
