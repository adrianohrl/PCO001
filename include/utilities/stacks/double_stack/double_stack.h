...
class DoubleStack : public DoubleStackInterface<Element>
{
public:
  DoubleStack(const unsigned int max_size);
  DoubleStack(const DoubleStack<Element>& stack);
  virtual ~DoubleStack();
...
template <typename Element>
DoubleStack<Element>::DoubleStack(const unsigned int max_size)
    : max_size_(max_size)
{
  top_A_ = -1;
  top_B_ = max_size_;
...
template <typename Element>
DoubleStack<Element>::DoubleStack(const DoubleStack& stack)
    : max_size_(stack.max_size_)
{
  ...
  for (int i = 0; i < max_size_; i++)
  {
    if (stack.vector_[i])
    {
      vector_[i].set(stack.vector_[i].element_, stack.vector_[i].previous_);
    }
  }
}
...
template <typename Element>
std::string DoubleStack<Element>::toString(int index)
{
  std::string str = "";
