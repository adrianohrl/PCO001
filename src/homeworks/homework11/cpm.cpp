...
CPM::CPM(std::string file_name, std::string time_unit)
    : graph_(NULL), time_unit_(time_unit)
{
  if (!exists(file_name))
  {
...
std::list<Activity> CPM::getCriticalPath()
{
  if (graph_->isEmpty())
  {
    throw utilities::Exception("In order to calculate the critical path, the "
                               "CPM graph must not be empty!!!");
  }
  if (!graph_->isDirected())
  {
    throw utilities::Exception("In order to calculate the critical path, the "
                               "CPM graph must be a directed graph!!!");
  }
  if (!graph_->isAcyclic())
  {
    throw utilities::Exception("In order to calculate the critical path, the "
                               "CPM graph must be an acyclic graph!!!");
  }
  ...
  std::list<std::pair<Activity, double> > predecessors(
      graph_->getPredecessors(activity));
...
double CPM::getTotalDuration() { return getTotalDuration(getCriticalPath()); }
...
void CPM::import(std::string file_name)
{
  ...
  while (file.good())
  {
    ...
    if (!line.empty() && line[0] != '#')
    {
      ...
      activities.insert(
          std::pair<char, Activity>(id, Activity(id, name, duration)));
      if (columns[3][0] != '-')
      {
        ...
        if (!ret.second)
        {
          ...
          throw utilities::Exception(
              "Multiple entries of an existing activity ID!!!");
        }
      }
    }
    std::getline(file, line);
...
Activity CPM::getActivity(char id) const { return getActivity(Activity(id)); }

std::string CPM::getTimeUnit() const { return time_unit_; }
...
std::string CPM::str() const
{
  ...
  while (activities_it != activities.end())
  {
    ...
    aux << activity.getName() << " (" << activity.getDuration() << " ["
        << time_unit_ << "])";
    ...
    predecessors.insert(std::pair<char, std::vector<char> >(
        activity.getId(), std::vector<char>()));
    activities_it++;
  }
  ...
  {
    Activity activity(*activities_it);
    std::list<std::pair<Activity, double> > successors(
        graph_->getArcs(activity));
    std::list<std::pair<Activity, double> >::const_iterator successors_it(
        successors.begin());
    while (successors_it != successors.end())
    {
      Activity successor(successors_it->first);
      std::map<char, std::vector<char> >::iterator predecessors_it(
          predecessors.find(successor.getId()));
  ...
  while (activities_it != activities.end())
  {
    ...
    std::vector<char> activity_predecessors(
        predecessors.find(activity.getId())->second);
...
bool CPM::exists(std::string file_name) const { return true; }

void CPM::buildGraph(std::map<char, Activity> activities,
                     std::map<char, std::vector<char> > predecessors)
{
  if (graph_)
  {
  ...
  graph_ =
      new utilities::graphes::AdjacencyList<Activity>(MAX_NUM_ACTIVITIES, true);
  ...
  while (predecessors_it != predecessors.end())
  {
    ...
    for (int i(0); i < pred.size(); i++)
    {
      ...
      std::map<char, Activity>::iterator destiny(
          activities.find(predecessors_it->first));
...
double CPM::max(double d1, double d2) const { return d1 > d2 ? d1 : d2; }
}
}
